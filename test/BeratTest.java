...
public class BeratTest {
    ...
    @Test
    public void testCheckInPatron() throws SQLException {
        ...
        expectedBooking.setStatus("Checked In"); //set the Status of the EXPECTED BOOKING to 'Checked In'
        testPayment.testCheckInPatron("Checked In"); //update the ACTUAL BOOKING status to 'Checked In' using DB Operations
        actualBooking = testPayment.returnBooking(); //Return the ACTUAL BOOKING to actual booking object
        assertEquals(expectedBooking.getStatus(), actualBooking.getStatus()); //Check whether expected status and actual status are the same ('Checked In')
    }
